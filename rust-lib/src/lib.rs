...
impl EGraph {
    ...
    // method argument: https://pyo3.rs/v0.4.1/print.html
    #[args(exprs = "*")]
    ...
    fn run(
        ...
        // the default scheduler is BackoffScheduler
        ...
        // let egraph_nodes: usize = runner.egraph.total_number_of_nodes();
    ...
    fn num_enodes(&mut self) -> usize {
        // this is the effective enode
        *&self.egraph.total_number_of_nodes()
    }

    fn total_size(&mut self) -> usize {
        // this is the actual number of Enodes, including outdated ones
        // and it is the node-lim that stop equality saturation
    ...
    // for pickleable in python
    // NOTE: it is currently not possible to de/serialize a egraph
    // seek solution in python level
    // pub fn __setstate__(&mut self, py: Python, state: PyObject) -> PyResult<()> {
    //     match state.extract::<&PyBytes>(py) {
    //         Ok(s) => {
    //             self.egraph = deserialize(s.as_bytes()).unwrap();
    //             Ok(())
    //         }
    //         Err(e) => Err(e),
    //     }
    ...
    //     Ok(PyBytes::new(py, &serialize(&self.egraph).unwrap()).to_object(py))
    // }
...
#[derive(Clone)]
pub struct ASTcost;
impl<L: egg::Language> egg::CostFunction<L> for ASTcost {
    type Cost = usize;
...
fn rust_lib(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(print_id, m)?)?;
