        ...
        if action is None:
            ...
            for idx, a in enumerate(actor_action):
                ...
                if a == self.actor_n_action - 1:
                    ...
                    # sample-basd
                    loc_probs = torch.nn.functional.softmax(
                        torch.tensor(loc_scores), dim=0)
                    loc_dist = Categorical(probs=loc_probs)
                    sampled_loc_action = int(loc_dist.sample())
                    action = (a, sampled_loc_action)
                    loc_vf = loc_scores[sampled_loc_action] * loc_probs[
                        sampled_loc_action]

                output_action.append(action)
                output_vf.append(loc_vf)

            output_action = torch.tensor(output_action,
                                         dtype=torch.int8).to(self.device)
            output_vf = torch.tensor(output_vf,
                                     dtype=torch.float32).to(self.device)
            return output_action, probs.log_prob(
                sampled_actor_action), None, output_vf
        ...
        # training
            ...
            for idx in range(bs):
                ...
                if rule_id == self.actor_n_action - 1:
                    ...
                    pmaps = batch_pattern_map[idx][rule_id]
                    loc_scores = []
                    for pmap in pmaps:
                        loc_score = 0.
                        for _, v in pmap.items():
                    ...
                    loc_probs = torch.nn.functional.softmax(
                        torch.tensor(loc_scores), dim=0)
                    score = loc_scores[loc_id] * loc_probs[loc_id]

                output_vf.append(score)

            output_vf = torch.tensor(output_vf,
                                     dtype=torch.float32).to(self.device)
            return None, probs.log_prob(action[:,
                                               0]), probs.entropy(), output_vf
